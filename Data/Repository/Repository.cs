...
namespace ZingCRM_Demo.Data.Repository
{
    public class Repository<T>(IDbContextFactory<ApplicationDbContext> contextFactory) : IRepository<T> where T : class
    {
        ...
        public async Task<List<T>> GetAllAsync(Expression<Func<T, object>> orderBy = null, bool ascending = true, Expression<Func<T, bool>> filter = null, Expression<Func<T, object>> selector = null, string includeProperties = null, int pageSize = 0, int pageNumber = 1, bool isTracked = false)
        {
            try
            {
                ...
                IQueryable<T> query = isTracked ? context.Set<T>() : context.Set<T>().AsNoTracking();
        ...
        public async Task<T> GetAsync(Expression<Func<T, object>> orderBy = null, bool ascending = true, Expression<Func<T, bool>> filter = null, string includeProperties = null, bool isTracked = true)
        {
            try
            {
                using var context = _contextFactory.CreateDbContext();
